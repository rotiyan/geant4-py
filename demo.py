...
def Configure():
  ...
  # ------------------------------------------------------------------
  #gRunManager.SetUserInitialization((G4physicslists.FTFP_BERT()))
  gRunManager.SetUserInitialization((G4physicslists.QGSP_BERT_HP()))
...
#title and header    row=0, 1
    title = Label(self, text="Geant4Py Author H. Yoshida Naruto Univ. of Education (Modified by R. Narayan)")
    title.grid(row=0, column=1, columnspan=5)
    header = Label(self, text="Interaction of particles with matter")
...
#absorber thickness row=3
    ...
    thick = Entry(self,textvariable=self.thickVar)
...
#particle row=4
    ...
    ra1 = { }
    pos1=1
    for i in ("gamma", "e-","mu-","tau-","nu_e","proton","neutron","alpha","He3","deutron"):
    ...
    self.energyVar.set(1000)
    energy = Entry(self,textvariable=self.energyVar)
...
#number of event row=6 
    ...
    self.eventVar.set(10)
    event = Entry(self,textvariable=self.eventVar)
...
# Rotate
    self.rotTheta=80
    self.rotPhi=10
    rotLabel = Label(self,text="rotate",bg="orange")
    rotLabel.grid(row=9,column=0,sticky=W)
    thetaBut = Button(self,text="rotate(theta)",command=self.cmd_rotateTheta)
    phiBut   = Button(self,text="rotate(ph)",command=self.cmd_rotatePhi)
    resetBut = Button(self,text="reset",command=self.cmd_rotateReset)
    thetaBut.grid(row=9,column=1,sticky=W)
    phiBut.grid(row=9,column=2,sticky=W)
    resetBut.grid(row=9,column=3,sticky=W)
